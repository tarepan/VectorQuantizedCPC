from dataset import dataset_adress
...
from preprocess import process_to_mel_mu
...
def get_dataset_item_id_list(dir_dataset: Path) -> Path:
    """Get the item list in dataset.
    """
    return dir_dataset / "id_list.pickle.bin"

def get_dataset_mulaw_path(dir_dataset: Path, item_id: ItemIdJVS) -> Path:
    """Get μ-law discrete waveform item path in dataset.
    """
    return dir_dataset / item_id.subtype / f"spk_{item_id.speaker}" / "mulaws" / f"utt_{item_id.serial_num}.mulaw.npy"


def get_dataset_mel_path(dir_dataset: Path, item_id: ItemIdJVS) -> Path:
    """Get mel-spec item path in dataset.
    """
    return dir_dataset / item_id.subtype / f"spk_{item_id.speaker}" / "mels" / f"utt_{item_id.serial_num}.mel.npy"


DatumJVS = Tuple[LongTensor, FloatTensor, int]

class JVSMulawMelSpkDataset(Dataset[DatumJVS]):
    """Audio mu_law_wave/mel_spec/speaker_index dataset from JVS corpus.
    """
    def __init__(self, train: bool, conf: ConfDataset):
        ...
        adress_archive, self._path_contents = dataset_adress(conf.adress_data_root, "JVS", "mel_mulaw", arg_hash)
        ...
        if not contents_acquired:
            ...
            with open(get_dataset_item_id_list(self._path_contents), mode='rb') as f:
                self._ids: List[ItemIdJVS] = pickle.load(f)
        self._speakers: List[int] = list(set(map(lambda item_id: item_id.speaker, self._ids)))
    ...
    def _load_datum(self, id: ItemIdJVS) -> DatumJVS:
    ...
    def __getitem__(self, n: int) -> DatumJVS:
        """Load n-th datum.
        Args:
